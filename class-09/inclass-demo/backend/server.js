...
// /////////////////////////////////////////////
// promises, an FYI
// promises, declarative functionality.  PROMISES ARE KEPT.  satisfied with resolution  OR rejection.  asynchronous  use .then()
...
function runsLong(status) {
  ...
  let myPromise = new Promise((resolve, reject) => {
    if (status) {
      setTimeout(() => {
        resolve('done after some time');
      }, 4000, 'it works');
    } else {
      // handling rejection...  won't happen because of setTimeout
...
function runsLonger(status) {
  // create a Promise
  let myPromise = new Promise((resolve, reject) => {
    if (status) {
      setTimeout(() => {
        resolve('done after some more time');
      }, 5000);
    } else {
      // handling rejection...  won't happen because of setTimeout
      reject('something went wrong!');
...
runsLong(' this totally works NOW')
  .then(returnValue => console.log(returnValue));

runsLonger(' this totally works I hope')
  .then(returnValue => console.log(returnValue));
